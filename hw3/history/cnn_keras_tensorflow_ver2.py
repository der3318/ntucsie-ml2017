...
with open(sys.argv[1], "r") as fin:
    for lineNum, line in enumerate(fin):
        if lineNum > 0:
            ...
            if int(line.rstrip().split(",")[0]) in do_flip:
                flip_index.append(lineNum - 1)
            Y_train.append(oneHot)
X_train = np.array(X_train) / 255.
X_train = X_train.reshape(X_train.shape[0], img_rows, img_cols, 1)
if flag_flip:
    X_train_flip = X_train[flip_index, :, ::-1, :]
    X_train = np.concatenate( (X_train, X_train_flip), axis = 0 )
rnd_perm = np.random.permutation(X_train.shape[0])
X_train = X_train[rnd_perm, :, :, :]
Y_train = np.array(Y_train)
if flag_flip:
    Y_train = np.concatenate( (Y_train, Y_train[flip_index, :]), axis = 0 )[rnd_perm, :]
else:
