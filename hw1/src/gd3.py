...
for i in range(1, maxIt + 1):
    if (i % 10000) == 0:
        ...
        for j in range(trainSize):
            trainX = dataNormed[j:j + days - 1,:].reshape(-1)
            trainY = data[j + days - 1, -1]
    ...
    for bat in range(batchSize):
        x = dataNormed[rndPerm[curIdx + bat]:rndPerm[curIdx + bat] + days - 1,:].reshape(-1)
        y = dataNormed[rndPerm[curIdx + bat] + days - 1, -1]
...
lost = 0.0
for j in range(data.shape[0] - 1000 - days + 1, data.shape[0] - days + 1):
    trainX = dataNormed[j:j + days - 1,:].reshape(-1)
    trainY = data[j + days - 1, -1]
    lost += ( trainY - maxPM * ( b + numpy.dot(trainX, w) ) ) ** 2
lost = lost / 1000
